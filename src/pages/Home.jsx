...
- And give the whole code in a single HTML file.
      ...
      if (error?.error?.code === 503) {
        toast.error(
          "API Server overloaded. Please wait a moment and try again."
        );
      } else if (error?.error?.code === 429) {
        toast.error("Too many requests. Please wait before trying again.");
      } else if (error?.error?.code === 401) {
        toast.error("API Key invalid. Please check your credentials.");
      } else {
        toast.error(
  ...
  return (
    <>
      ...
      >
        {/* Responsive layout */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 px-6 lg:px-16 py-8">
          {/* Left Section */}
          <div
            className={`w-full py-6 rounded-xl transition-colors duration-300 ${
              isDarkMode
                ? "bg-gray-900 border-gray-700"
                : "bg-white border-gray-200"
            } mt-5 p-5 border shadow-lg`}
          >
            ...
            <textarea
              ...
              value={prompt}
              className={`w-full min-h-[200px] rounded-xl mt-3 p-3 outline-none focus:ring-2 focus:ring-purple-500 resize-none transition-colors ${
                isDarkMode
          ...
          {/* Right Section */}
          <div
            className={`relative mt-2 w-full h-[80vh] rounded-xl overflow-hidden transition-colors ${
              isDarkMode
                ? "bg-[#141319] border-gray-700"
                : "bg-white border-gray-200"
            } border shadow-lg`}
          >
            {!outputScreen ? (
